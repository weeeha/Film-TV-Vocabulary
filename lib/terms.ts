...
export function parseFilters(
  ...
  return {
    q: (params.get("q") ?? "").trim(),
    chapter: ids.includes(chapter) ? chapter : "",
  };
}
export function filterTerms(
  ...
  return terms
    .filter(
      (t) =>
        (!filters.chapter || t.chapter_id === filters.chapter) &&
        normalize(t.name).includes(q),
    )
    .sort(
      (a, b) =>
        a.name.localeCompare(b.name, "en") ||
        a.chapterTitle.localeCompare(b.chapterTitle, "en") ||
...
export function groupTerms(terms: TermListItem[]): Map<string, TermListItem[]> {
  ...
  for (const term of terms) {
    const first = normalize(term.name).charAt(0).toUpperCase();
