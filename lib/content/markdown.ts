...
export const legacyAnchor = (text: string) =>
  text
    .replace(/[`*_]/g, "")
    .toLowerCase()
    .trim()
...
export function prepareDocument(
  ...
  const counter = new Map<string, number>(),
    headingMap = new Map<RootContent, string>();
  for (const node of tree.children)
    if (node.type === "heading") {
      const base = legacyAnchor(toString(node)),
        count = counter.get(base) || 0;
      counter.set(base, count + 1);
      const id = base + (count ? "-" + count : "");
      anchors.push(id);
      headingMap.set(node, id);
      if (node !== h1) headingIds.push(id);
    }
    ...
    if (n.type === "html") {
      ...
      if (id) {
        ...
        if (!heading || heading.type !== "heading" || heading.depth !== 3)
          throw new Error(sourcePath + ": term ID without heading");
        const e = c.entries.find((e) => e.id === id),
          headingId = headingMap.get(heading)!;
        if (
          !e ||
          e.source_path !== sourcePath ||
          e.name !== toString(heading) ||
          e.anchor !== headingId
        )
          throw new Error(sourcePath + ": catalog/heading disagreement " + id);
        termAnchors[headingId] = id;
        anchors.push(id);
      } else if (!/^<!--[\s\S]*-->$/.test(n.value))
        throw new Error(sourcePath + ": unsupported HTML");
    }
  }
  if (new Set(anchors).size !== anchors.length)
    throw new Error(sourcePath + ": duplicate anchor");
  tree.children = tree.children.filter((n) => {
    if (n === h1 || n.type === "html") return false;
    if (n.type === "paragraph" && toString(n).startsWith("Wiki home"))
      return false;
    if (
      !r.has(pilotPath) &&
      n.type === "paragraph" &&
      toString(n).startsWith("Illustrated pilot:")
    )
      return false;
    return true;
  });
  visit(tree, (n, index, parent) => {
    ...
    if (n.type === "link" || n.type === "image" || n.type === "definition") {
      n.url = resolveLink(sourcePath, n.url, r);
      links.push(n.url);
    }
    if (n.type === "html")
      throw new Error(sourcePath + ": unsupported inline HTML");
  });
  ...
  const flush = () => {
    ...
    if (text && !isTerm)
      sections.push({
        id: sourcePath + "#" + sectionId,
        title: sectionTitle,
        context: title.replace(/^\d+\s*·\s*/, ""),
        text,
  ...
  for (const n of tree.children) {
    if (n.type === "heading") {
      flush();
      sectionTitle = toString(n);
      sectionId = headingMap.get(n)!;
      isTerm = !!termAnchors[sectionId];
  ...
  return {
    ...
    title,
    titleId: h1 ? headingMap.get(h1)! : legacyAnchor(title),
    summary: first ? toString(first) : "",
    markdown: unified()
