...
test('home presents the atlas and navigable chapters',async({page})=>{
 await page.goto('/');
 await expect(page.getByRole('heading',{name:'Find the words. Shape the scene.'})).toBeVisible();
 await expect(page.locator('main').getByRole('link',{name:/Lighting and color/})).toBeVisible();
});
test('chapter prose and both term addresses are available',async({page})=>{
...
test('unknown routes provide navigation instead of a blank page',async({page})=>{
 ...
 await expect(page.getByRole('link',{name:'Back to the atlas'})).toBeVisible();
});
test('reading remains usable without JavaScript',async({browser,baseURL})=>{
 const context=await browser.newContext({javaScriptEnabled:false,baseURL});
 const page=await context.newPage();await page.goto('/wiki/lighting-and-color');
 await expect(page.getByText('The principal source or contribution',{exact:false})).toBeVisible();
