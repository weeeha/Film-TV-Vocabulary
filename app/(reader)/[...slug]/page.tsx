...
export async function generateMetadata({
  ...
  const page = atlas.pages.find((p) => p.slug.join("/") === slug.join("/"));
...
export default async function Page({
  ...
  if (!page) notFound();
  const prepared = atlas.pages.find((p) => p.url === page.url);
  if (!prepared) notFound();
  const Body = page.data.body;
  return (
    <DocsPage toc={page.data.toc}>
      <DocsTitle id={prepared.titleId}>{prepared.title}</DocsTitle>
      <DocsBody className="atlas-prose">
        <Body components={getMDXComponents(prepared)} />
      </DocsBody>
      <noscript>
        <nav aria-label="Chapter index">
          <h2>Explore more chapters</h2>
