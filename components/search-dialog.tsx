...
export default function AtlasSearchDialog(props: SharedProps) {
  ...
  useEffect(() => {
    ...
    loadSearchRecords().then(
      (records) => {
        if (live) setState({ status: "ready", records });
      },
      () => {
  ...
  const found =
    state.status === "ready" ? searchRecords(state.records, query) : [];
  const items: SearchItemType[] = found.map((r) => ({
    id: r.id,
    url: r.url,
    type: "page",
    content: r.title,
    ...
    >
      ...
      >
        <SearchDialogHeader>
          ...
          />
          <SearchDialogClose>Esc</SearchDialogClose>
        </SearchDialogHeader>
        {state.status === "loading" ? (
          ...
          <div className="search-message" role="alert">
            <p>Search could not load. Try again or browse a chapter.</p>
            <button
            ...
            <a href="/">Browse chapters</a>
          </div>
        ) : !query.trim() ? (
          <p className="search-message">
            Search terms, definitions, and examples.
          </p>
        ) : (
          <SearchDialogList
            role="listbox"
            ...
            Item={({ item, onClick }) => {
              const record = found.find((r) => r.id === item.id)!;
              return (
                <SearchDialogListItem
                  item={item}
                  onClick={onClick}
